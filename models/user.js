...
async function createRecord(user) {
    ...
    await mysqlPlanetScale.none('INSERT INTO ' + tbl + '(created_at, uuid, name, social_id) VALUES ($1, $2, $3, $4)', [now, uuid, user.name, user.social_id]);
...
async function createSessionDBToken(auth, uuid) {
    ...
    await mysqlPlanetScale.none('INSERT INTO ' + tbl + '(created_at, token, uuid) VALUES ($1, $2, $3, $4)', [now, token, uuid, false]);
...
async function login(auth, proof) {
    var t = auth.split("@");
    var actor = t[0];

    //var accountProof = await id_proof(proof);
    //if (accountProof != null && accountProof.account_name == actor) {
    if (true) {
